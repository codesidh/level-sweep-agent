 ...
 * {@link TradeProposed} record, deserialized via Quarkus's auto-configured
 * Jackson {@code ObjectMapper}.
 *
 * <p>Companion to
 * {@code com.levelsweep.decision.saga.TradeProposedKafkaPublisher} on the
 ...
 * but consumers need an explicit type binding because Kafka hands us
 * {@code byte[]} with no type information attached.
 *
 * <p>Wiring: referenced from {@code application.yml} as the
 * {@code value.deserializer} for the {@code trade-proposed-in} incoming
 * channel. Quarkus instantiates one per channel via the standard Kafka
 * deserializer reflection pathway (no-arg constructor required), which the
 * {@link #TradeProposedDeserializer()} no-arg constructor satisfies.
 *
 ...
 * by Quarkus version). Mirrors decision-engine's {@code BarDeserializer}. See:
 * https://quarkus.io/guides/kafka#kafka-serialization
 */
public class TradeProposedDeserializer extends ObjectMapperDeserializer<TradeProposed> {

    public TradeProposedDeserializer() {
        super(TradeProposed.class);
    }
}
